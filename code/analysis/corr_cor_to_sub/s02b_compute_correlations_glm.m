...
for subj=1:8 
	...
	for hemi=1:2
		...
		for label=1:14
			...
			corr = f_calc_glm(x(:, 1:(750*n_sess_per_subj(subj)))', data_cortical(1:(750*n_sess_per_subj(subj)), :)); 

            % Output `corr` is (56*22*27, 15) matrix; we will loop through the columns and
            % save them as a NIFTI file
            for idx_beta = 1:15
                ...
                subvol(:) = corr(:, idx_beta);
                vol(coords_start(1):coords_end(1), ...
                    coords_start(2):coords_end(2), ...
                    coords_start(3):coords_end(3)) = subvol;

                % Save to disk as nifti 
                nsd_savenifti(vol, [1, 1, 1], [data_dir 'subj0' num2str(subj) '/mni/corr_cor_to_sub_hemi_' num2str(hemi) '_label_' num2str(label) '_method_1_glm_beta=' num2str(idx_beta) '.nii.gz'], 1, [92, 127, 73]);
            end
			...
			for repeat=1:6
				...
				corrs(:, :, repeat) = ...
                    f_calc_glm(x(:, idxs_sub)', data_cortical(idxs_cor, :));  
			end
			corr = nanmean(corrs, 3);
            ...
            for idx_beta = 1:15
                ...
                % Save to disk as nifti 
                nsd_savenifti(vol, [1, 1, 1], [data_dir 'subj0' num2str(subj) '/mni/corr_cor_to_sub_hemi_' num2str(hemi) '_label_' num2str(label) '_method_2_glm_beta=' num2str(idx_beta) '.nii.gz'], 1, [92, 127, 73]);
            end
		end
	end
end
