...
% based on the first round of reviews and attempts to use semi-partial correlation to
% isolate unique contributions of cortical seed areas. 
%
% Depends
...
for subj=1:8 
	...
	for hemi=1:2
		...
		for label=1:14
			...
			fprintf(['Subj: ' num2str(subj) ', Hemi: ' num2str(hemi) ', Label: ' num2str(label) '\n']);

            % Break cortical data into seed of interest (indexed by label) and other seeds, which will be treated as regressors in semi-partial correlation
            data_cortical_target = squeeze(data_cortical(:, label));
            data_cortical_regressors = data_cortical(:, 1:14 ~= label);

			% ===== METHOD #1 =====
			% Calculate semi-partial correlations coefficients
			...
			nsd_savenifti(vol, [1, 1, 1], [data_dir 'subj0' num2str(subj) '/mni/corr_cor_to_sub_hemi_' num2str(hemi) '_label_' num2str(label) '_method_1_semipartial.nii.gz'], 1, [92, 127, 73]);
			...
			% Save to disk as nifti 
			nsd_savenifti(vol, [1, 1, 1], [data_dir 'subj0' num2str(subj) '/mni/corr_cor_to_sub_hemi_' num2str(hemi) '_label_' num2str(label) '_method_2_semipartial.nii.gz'], 1, [92, 127, 73]);
		end
	end
end
